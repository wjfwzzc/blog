...
{% highlight shell linenos %} sudo python manage.py syncdb {% endhighlight %}
...
{% highlight shell linenos %} mysql -u root -p {% endhighlight %}
{% highlight sql linenos %} create databases oj3rd; {% endhighlight %}
之后再退出来运行syncdb语句，我们发现成功生成了若干个表。但我们运行
{% highlight shell linenos %} sudo python manage.py validate {% endhighlight %}
...
{% highlight shell linenos %} sudo python manage.py runserver {% endhighlight %}
...
之后重启Nginx
{% highlight shell linenos %} sudo /etc/init.d/nginx restart {% endhighlight %}
星爷把运行FastCGI和运行Django的一切工作都写进了reload.sh，所以直接运行reload.sh，在浏览器中输入localhost应该就能看到效果了。
如果想要查看Memcached的运行状态，可以输入
{% highlight shell linenos %} telnet localhost 11211 {% endhighlight %}
然后就可以输入stats查询了。
