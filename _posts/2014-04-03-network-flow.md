...
- 算法上，设某图点数为V，边数为E。邻接矩阵+裸Dijkstra复杂度是$$O(V^2)$$，邻接表+Dijkstra+priority_queue的复杂度是$$O((V+E)logV)$$，速度上是最稳定的；Barty的板子是Dijkstra+heap，但heap用set去实现，复杂度相同但实测速度上要慢上一些。前向星+SPFA+SLF+LLL的理论复杂度依旧是最快的$$O(kE)$$，但是常数k不太稳定，有退化成$$O(VE)$$的可能（其实可能性很小）；其中两个优化同时使用效果会好一些。
